...
public class Mon_Boss extends Monster implements Actable {
    ...
    public void move() {
        ...
        if (up && isRunning && !isDying) velocity = velocity.add(UP_DIR);
        if (down && isRunning && !isDying) velocity = velocity.add(DOWN_DIR);
        if (left && isRunning && !isDying) velocity = velocity.add(LEFT_DIR);
        if (right && isRunning && !isDying) velocity = velocity.add(RIGHT_DIR);
